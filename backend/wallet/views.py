...
def load_private_key_from_env():
    ...
    Handles multiple formats of newlines.
    """
    ...
    # Fix newlines — Render/Railway may store as literal \n
    # Try multiple replacements
    pem = pem.replace('\\n', '\n')  # literal \n → real newline
    pem = pem.replace('\r\n', '\n')  # Windows newlines
    pem = pem.strip()

    # If key doesn't have proper header — it's base64 encoded
...
if RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET:
    ...
    print("WARNING: Razorpay keys not set — using mock bank")
...
if _loaded_key:
    ...
    # Encode as base64 for easy copy-paste to Render
...
def create_payment_order(request):
    ...
    Phone opens Razorpay checkout with this order.
    """
...
def verify_payment(request):
    ...
    After user pays in Razorpay checkout,
    phone sends payment details here.
    We verify and load money to wallet.
    """
    ...
    # Payment verified — load money to wallet
    amount_decimal = Decimal(str(float(amount) / 100))  # convert paise to rupees
...
def razorpay_cashout(request):
    """
    Send money from wallet to user's bank account.
    Uses Razorpay Payout API.
    ...
    if razorpay_client is None or not account_number:
        # Mock cashout — just reset wallet
        # Credit back to mock bank account
        try:
    ...
    try:
        ...
        payout = razorpay_client.payout.create({
            'account_number': '2323230060659370',  # your RazorpayX account
            'fund_account': {
